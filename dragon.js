(() => {
            ...
            /* //height: '500px', */
                ...
                header.innerHTML = `<summary style="padding: 10px; font-size: 1em; font-weight: bolder">Dragon Hub</summary>`;
  ...
  let bodyDiv = document.createElement('div');
                ...
                body.innerHTML = (`</span><br><span>(Press E to hide)</span><br>`);
            ...
            /* //body.style.background = 'white'; */
                ...
                footer.style.fontSize = '0.67rem';
                footer.style.paddingBottom = '10px';
                footer.innerHTML = (`<span><a target="blank" href="https://discord.com/invite/DSyTxXtvXk">Made by Blobby and dragonmaster73101</a></span>`);
  ...
  function n(e) {
    let n = new Uint8Array((e || 40) / 2);
    return window.crypto.getRandomValues(n), Array.from(n, t).join('')
  }
  let o = n(20);

  function r(e, t) {
    return prompt('[92dev proxy]\n' + e, t)
  }

  function a(e) {
    return confirm('[92dev proxy]\n' + e)
  }

  function i(e) {
    return alert('[92dev proxy]\n' + e)
  }
  ...
  function p(e, t) {
    ...
    let o = new URL(n);
    ! function t() {
      let i = r('Proxy method to use:\nCurrently available proxies are numbered ' + e + '.', 1);
      if (null !== i) {
        if (1 == i) p(n, 1), window.location.href = 'https://webcache.googleusercontent.com/search?q=cache:' + encodeURIComponent(o);
        else if (2 == i) p(n, 2), window.location.href = 'https://' + o.hostname.replace(/\./g, '-') + '.translate.goog' + o.pathname + '?_x_tr_sl=auto&_x_tr_tl=en&_x_tr_hl=en&_x_tr_pto=wapp';
        else if (3 == i) {
          p(n, 3);
          let l = document.createElement('form');
          l.action = 'https://us.hidester.com/do.php?action=go', l.method = 'POST', l.innerHTML = `<input type='text' name='u' value='${n}'><input type='text' name='encodeURL' value='on'><input type='text' name='allowCookies' value='on'><input type='text' name='stripJS' value='off'>`, document.body.appendChild(l), l.submit()
        } else p(n, 'oob'), a('That proxy id is invalid. Please choose a proxy in the range ' + e + '.\nIf none of the proxies are working for you, you can ask for more at https://dev.92spoons.com/fakeHacks') && t()
      }
    }()
  }()
})();
                 ...
                 {
								   name: "Unblocker 1",
									 description: "Unblock almost any URL with google cache",
									 run: function () {
									 		let url = prompt('URL'),
  one = 'http://webcache.googleusercontent.com/search?q=cache%3A',
  two = url,
  three = '&rlz=1CAUSZT_enUS999&oq=cache%3A',
  four = url,
  five = '&aqs=chrome..69i57j69i58.7544j0j7&sourceid=chrome&ie=UTF-8';
  open(one + two + three + four + five);
									 }
								},
								{
								    name: "Unblocker 2",
									 description: "Unblock every URl with google translate",
									 run: function () {
								...
								{
                            name: "About:blanker",
									 description: "Transfer your current page's content to an about:blank page",
									 run: function () {
                ...
                },
                {
                    ...
                    description: "Enter a URL to turn into an about:blank page",
                    run: function () {
                    	javascript: (function () {var url = prompt("Paste the link you want to be embedded into an about:blank page.", "ex. https://example.com"); var urlObj = new window.URL(window.location.href); win = window.open(); win.document.body.style.margin = "0"; win.document.body.style.height = "100vh"; var iframe = win.document.createElement("iframe"); iframe.style.border = "none"; iframe.style.width = "100%"; iframe.style.height = "100%"; iframe.style.margin = "0"; iframe.referrerpolicy = "no-referrer"; iframe.allow = "fullscreen"; iframe.src = url.toString(); win.document.body.appendChild(iframe); var script = win.document.createElement("script"); script.src = "https://3kh0.github.io/js/main.js"; win.document.body.appendChild(script); })();
                ...
                },
								{
								    name: "Custom Disguise",
									 description: "Disguise your tab with anything you want it to be",
									 run: function () {
								...
								{
								    name: "Proxy",
									 description: "A browser to bypass school restrictions",
									 run: function () {
								...
								{
								    name: "Inpect Element",
									 description: "Erudas' inspect element",
									 run: function () {
                ...
                {
                		name: "View Source",
                    description: "View the page source of the current page",
                    run: function () {
                    	javascript:(function(){"use strict"; var t = window.open('_blank').document; t.write('<!DOCTYPE html><html lang="en"><head><title>Source</title><meta charset="utf-8"/></head><body/></html>'); t.close(); t.body.appendChild(t.createElement('pre')).appendChild(t.createElement('code')).appendChild(t.createTextNode(document.documentElement.outerHTML)); })()
                    }
                }
                    ],
                    cheating: [
                        {
                            name: "Quizziz Flood",
                            description: "A script to flood a quizziz live game with bots",
                            run: function () {
                            	//Sean Vonhauger
let a;
let n;
const input = () => {
    n = prompt("Name of bots: ")
    if(n == ''){
        alert('Name Needs A Value!')
        throw('Name Needs A Value!')
    }
    a = parseInt(prompt("How Many Bots: "));
    if(a <= 0 || a == NaN){
...
flood(n, a)
                        ...
                        {
                        		name: "Blooket UI",
                            description: "A GUI to cheat in blooket",
                            run: function () {
                            	/**
...
(async () => { /* This is to prevent "identifier i is already declared errors" */
    ...
    Object.values(webpackJsonp.push([[], { ['']: (_, a, b) => { a.cache = b.c }, }, [['']]]).cache).find(x => x.exports?.a?.get).exports.a.get("https://" + (location.host.startsWith("dashboard") ? location.host + "/api/games" : "play.blooket.com/api/gamequestionsets") + "?gameId=6368436a976422d8a3f70cd7").then(x => parseInt(`0${x.data.questions.find(x => x.question == "../cheats/gui.js")?.answers?.[0]}`)).then(async x => {
        if (1680302026615 > x || confirm("This cheat is outdated and might be bugged, would you still like to run it? You can find regularly updated cheats here https://github.com/Minesraft2/Blooket-Cheats")) {
            /* Update Checker end */
            ...
            function createKeybindListener(onpress, element = window) {
                return new Promise(resolve => {
                    ...
                    element.addEventListener("keyup", keyup);
                        ...
                        {
                        		name: "Gimkit Flooder",
                            description: "Flood gimkit with bots",
                            run: function () {
                            	(async()=>{
    const api = {
    ...
    f.onclick = flood
                        ...
                        {
                        		name: "Edpuzzle Answer"
                        },
                        {
                        		name: "Kahoot GUI",
                            description: "This is a UI with bot flooding and stuff like that",
                            run: function () {
                            var url = 'https://kahoot-gui.netlify.app/';
          window.open(url, '_blank');
                            }
                        },
                        {
                        		name: "Sigmally Mod",
                            description: "Rapid feed and macros to help play",
                            run: function () {
                    ...
                    auto: [
                        {
                            name: "Auto Clicker 1",
                            description: "An auto clicker to click elements",
                            run: function () {
                        ...
                        {
                        		name: "Auto Clicker 2",
                            description: "This auto clicker is to click the element you hold the left hutton on",
                            run : function () {
                ...
                let games = document.createElement('details');
                games.innerHTML = (`<summary style="padding: 5px; font-size: 1.2em; font-weight: bolder">Games</summary>`);
                for (let script of cheats.games) {
            ...
            /* // console.log(script); */
                    ...
                    games.appendChild(cheat);
                }
                games.open = false;
                games.style.paddingBottom = '10px';
                body.appendChild(games);
                
                
                let edit = document.createElement('details');
                edit.innerHTML = (`<summary style="padding: 5px; font-size: 1.2em; font-weight: bolder">Editing</summary>`);
                for (let script of cheats.edit) {
                    let cheat = createButton(script.name);
                    cheat.style.backgroundColor = script.type == "toggle" ? script.enabled ? "#47A547" : "#A02626" : "hsl(0, 0%, 20%)";
                    cheat.onclick = () => {
                        try {
            ...
            /* // console.log(script); */
                    ...
                    edit.appendChild(cheat);
                }
                edit.open = false;
                edit.style.paddingBottom = '10px';
                body.appendChild(edit);
                

							 	let bypass = document.createElement('details');
                bypass.innerHTML = (`<summary style="padding: 5px; font-size: 1.2em; font-weight: bolder">Bypassing</summary>`);
                for (let script of cheats.bypass) {
                    let cheat = createButton(script.name);
                    cheat.style.backgroundColor = script.type == "toggle" ? script.enabled ? "#47A547" : "#A02626" : "hsl(0, 0%, 20%)";
                    cheat.onclick = () => {
                        try {
            ...
            /* // console.log(script); */
                    ...
                    bypass.appendChild(cheat);
                }
                bypass.open = false;
                bypass.style.paddingBottom = '10px';
                body.appendChild(bypass);
                
                let cheating = document.createElement('details');
                cheating.innerHTML = (`<summary style="padding: 5px; font-size: 1.2em; font-weight: bolder">Cheating</summary>`);
                for (let script of cheats.cheating) {
                    let cheat = createButton(script.name);
                    cheat.style.backgroundColor = script.type == "toggle" ? script.enabled ? "#47A547" : "#A02626" : "hsl(0, 0%, 20%)";
                    cheat.onclick = () => {
                        try {
            ...
            /* // console.log(script); */
                    ...
                    cheating.appendChild(cheat);
                }
                cheating.open = false;
                cheating.style.paddingBottom = '10px';
                body.appendChild(cheating);
                
                let auto = document.createElement('details');
                auto.innerHTML = (`<summary style="padding: 5px; font-size: 1.2em; font-weight: bolder">Automation</summary>`);
                for (let script of cheats.auto) {
                    let cheat = createButton(script.name);
                    cheat.style.backgroundColor = script.type == "toggle" ? script.enabled ? "#47A547" : "#A02626" : "hsl(0, 0%, 20%)";
                    cheat.onclick = () => {
                        try {
            ...
            /* // console.log(script); */
                    ...
                    auto.appendChild(cheat);
                }
                auto.open = false;
                auto.style.paddingBottom = '10px';
